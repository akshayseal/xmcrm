...
const stateLayout = {
  "Jammu and Kashmir": [42, 6],
  Ladakh: [56, 7],
  "Himachal Pradesh": [45, 17],
  Punjab: [38, 22],
  Chandigarh: [43, 24],
  Haryana: [43, 29],
  Delhi: [47, 33],
  ...
  "Tamil Nadu": [58, 88],
  Telangana: [55, 66],
  "Andhra Pradesh": [62, 74],
  Odisha: [70, 62],
  Chhattisgarh: [61, 55],
  "Madhya Pradesh": [51, 49],
  "Uttar Pradesh": [57, 38],
  Uttarakhand: [52, 27],
  Bihar: [70, 39],
  Jharkhand: [70, 49],
      ...
      <section class="panel">
        <div class="panel-head">
          <div>
            ...
            <div class="muted">Hover a state for school count. Click a state, then choose a district to open its dataset.</div>
          </div>
          <span class="badge">${total} schools</span>
        </div>
        <div class="india-map" aria-label="India state school map">
          <div class="map-outline"></div>
          ${state.mapData.states.map(stateBubble).join("")}
        </div>
      </section>
      <section class="panel">
        <div class="panel-head">
        ...
        <div class="panel-body stack">
          ${state.selectedState ? districtList() : `<div class="empty">Click a state marker on the map to see districts.</div>`}
        </div>
      </section>
    </div>
  `;
...
function stateBubble(item) {
  const [x, y] = stateLayout[item.state] || [50, 50];
  const size = Math.max(28, Math.min(76, 26 + item.count / 2));
  return `
    <button class="state-bubble ${state.selectedState === item.state ? "active" : ""}"
      style="left:${x}%;top:${y}%;width:${size}px;height:${size}px"
      title="${escapeHtml(item.state)}: ${item.count} schools"
      onclick="selectMapState('${escapeAttr(item.state)}')">
      <span>${escapeHtml(shortState(item.state))}</span>
